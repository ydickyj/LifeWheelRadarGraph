...
public class VelocimeterView extends View {
  ...
  /*
    C1: public void VelocimeterView(Context context) {}
    你只打算用code动态创建一个view而不使用布局文件xml inflate，那么实现C1就可以了。
   */

  /*
  多了一个AttributeSet类型的参数，
  在通过布局文件xml创建一个view时，
  这个参数会将xml里设定的属性传递给构造函数。
  如果你采用xml inflate的方法却没有在code里实现C2，那么运行时就会报错。
  但是由于编译能顺利通过，对于我这样的菜鸟，这个错误有时不太容易被发现。
  */
